...
function startGrroxy(host) {
    ...
    const lorgPath = getBinPath('lorg')
    ...
    lorgProcess = spawn(lorgPath, ['start', '--host', host], {
        stdio: 'pipe',
        env: env,
        // On Windows, omit detached so lorg stays in Electron's Job Object
    ...
    lorgProcess.stdout.on('data', (data) => {
        console.log(`[lorg] ${data.toString().trimEnd()}`)
    })

    lorgProcess.stderr.on('data', (data) => {
        console.error(`[lorg] ${data.toString().trimEnd()}`)
    })

    lorgProcess.on('error', (err) => {
        console.error(`[lorg] Failed to start: ${err.message}`)
        lorgProcess = null
    })

    lorgProcess.on('close', (code) => {
        console.log(`[lorg] Process exited with code ${code}`)
        lorgProcess = null
    })
}
...
function stopGrroxy() {
    if (lorgProcess) {
        const pid = lorgProcess.pid
        lorgProcess = null
        // Kill the entire process tree (lorg + lorg-app + lorg-tool)
        if (process.platform === 'win32') {
            ...
            // Kill the process group (catches lorg-app + lorg-tool if same group)
...
function createWindow(lorgURL) {
    const iconPath = path.resolve(__dirname, "icons", "lorg.png")
...
app.whenReady()
    .then(async () => {
        ...
        console.log(`[electron] Starting lorg on ${host}`)
        startGrroxy(host)
