...
**Important**: External tools (like `lorg-tools`) use the SDK to update the main app's `_processes` collection, ensuring all processes are visible in the UI.

---

## Quick Start

### For External Tools (lorg-tools)

```go
import "github.com/campbellcharlie/lorg/internal/sdk"
...
### For Internal Use (lorg)

```go
import "github.com/campbellcharlie/lorg/internal/process"
...
```bash
export LORG_APP_URL="http://localhost:8090"
export LORG_ADMIN_EMAIL="admin@example.com"
export LORG_ADMIN_PASSWORD="your-secure-password"
```
...
tools.AppSDK = sdk.NewClient(
    ...
    sdk.WithAdminEmailPassword(
        os.Getenv("LORG_ADMIN_EMAIL"),
        os.Getenv("LORG_ADMIN_PASSWORD"),
    ),
)
...
func LoadConfig() (*ToolsConfig, error) {
    ...
    configPath := filepath.Join(home, ".config", "lorg", "tools.json")
...
if backend.AppSDK == nil {
    log.Fatal("SDK client not initialized. Set LORG_APP_URL and credentials.")
}
```
...
⚠️ **Restrict file permissions**: `chmod 600 ~/.config/lorg/tools.json`  
