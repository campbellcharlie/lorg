...
- Launches `lorg` process in background
...
Downloads and replaces all Grroxy binaries (`lorg`, `lorg`, `lorg-tool`) with the latest release.
...
```json
{
  ...
  "results": [
    ...
    },
    {
      "binary": "lorg-tool",
      "status": "skipped",
      "error": "no asset found matching \"lorg-tool-darwin-arm64\" in release v0.25.0"
    }
  ]
}
```
...
- File download of CA certificate (lorg-ca.crt)
...
2. **Launch**: Project automatically launches `lorg` with unique host address
3. **State Tracking**: Project state ("active"/"unactive") tracked in database
4. **Access**: Connect to project at the provided `ip` address
5. **Shutdown**: When `lorg` process terminates, state automatically set to "unactive"
...
- `LORG_PROJECTS_DIR` - Root directory for all projects
- `LORG_TEMPLATE_DIR` - Directory for template files
- `LORG_CONFIG_DIR` - Configuration directory
- `LORG_CACHE_DIR` - Cache directory
