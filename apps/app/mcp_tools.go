...
type ProxyStartArgs struct {
	...
	Browser string `json:"browser,omitempty" jsonschema:"enum=firefox,enum=none" jsonschema_description:"Browser to attach to the proxy instance: firefox (CamoFox, anti-detect, supports browser*/browserInteract/browserExec/etc. MCP tools) or none (default: none). For UI-driven Chrome use, start the proxy via the lorg UI."`
}
