...
func (backend *Backend) TrafficDetail(e *core.ServeEvent) error {
	e.Router.AddRoute(echo.Route{
		Method: http.MethodGet,
		Path:   "/api/traffic/:id/detail",
		Handler: func(c echo.Context) error {
			...
			reqRec, reqErr := dao.FindRecordById("_req", id)
			if reqErr == nil && reqRec != nil {
				rawReq = reqRec.GetString("raw")
			}
			respRec, respErr := dao.FindRecordById("_resp", id)
			...
			dataRec, dataErr := dao.FindRecordById("_data", id)
			if dataErr != nil {
