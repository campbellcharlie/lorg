...
func (backend *Backend) CWDContent(e *echo.Echo) {
	e.GET("/api/cwd", func(c echo.Context) error {
...
func (backend *Backend) CWDBrowse(e *echo.Echo) {
	e.POST("/api/cwd/browse", func(c echo.Context) error {
		if err := requireAuth(c); err != nil {
			return err
		}
		...
		basePath := path.Join(backend.Config.ProjectsDirectory, backend.Config.ProjectID)
		dirPath, _ := data["path"].(string)
		if dirPath == "" {
			dirPath = basePath
		} else {
			validated, err := validatePathContainment(basePath, dirPath)
			if err != nil {
				return c.JSON(http.StatusForbidden, map[string]interface{}{"error": "Access denied: path outside project directory"})
			}
			dirPath = validated
		}
...
func (backend *Backend) CWDReadFile(e *echo.Echo) {
	e.POST("/api/cwd/readfile", func(c echo.Context) error {
		if err := requireAuth(c); err != nil {
			return err
		}
		...
		basePath := path.Join(backend.Config.ProjectsDirectory, backend.Config.ProjectID)
		validated, err := validatePathContainment(basePath, filePath)
		if err != nil {
			return c.JSON(http.StatusForbidden, map[string]interface{}{"error": "Access denied: path outside project directory"})
		}
		filePath = validated
...
func (backend *Backend) FileWatcher(e *echo.Echo) {

	// SSE stream endpoint -- watches root + expanded dirs with debouncing
	e.POST("/api/filewatcher", func(c echo.Context) error {
		if err := requireAuth(c); err != nil {
			return err
		}
	...
	e.POST("/api/filewatcher/dirs", func(c echo.Context) error {
		if err := requireAuth(c); err != nil {
			return err
		}
