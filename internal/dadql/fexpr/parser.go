...
func Parse(text string) ([]ExprGroup, error) {
	...
	for {
		...
		case stepBeforeSign:
			...
			if t.Type == TokenLogicalOp && t.Literal == string(NotOperator) {
				step = 0
				result = append(result, ExprGroup{Join: join, Item: nil})
				join = getLogicalOp(t.Literal)
				continue
			}
		...
		case stepSign:
			...
			step = stepAfterSign
		case stepAfterSign:
			if t.Type != TokenIdentifier && t.Type != TokenRegex && t.Type != TokenText && t.Type != TokenNumber {
				return nil, fmt.Errorf("expected right operand (identifier, regex, text or number), got %q (%s)", t.Literal, t.Type)
			}
			...
			step = StepJoin
		case StepJoin:
			if t.Type != TokenLogicalOp {
				return nil, fmt.Errorf("expected AND or OR or NOT, got %q (%s)", t.Literal, t.Type)
			}
...
func getLogicalOp(literal string) LogicalOp {
	switch LogicalOp(literal) {
	case OrOperator:
		return "OR"
	case AndOperator:
		return "AND"
