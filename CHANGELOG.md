...
- **Intercepted landing page** — Browser now opens a custom `intercepted.html` page (served via `file://`) instead of `lorg.com`. So we don't capture unwanted traffic.
...
- **Electron: orphan child processes on quit** — Closing the Electron app now kills the entire process group (lorg, lorg, lorg-tool) instead of only the lorg process. Uses `detached: true` spawn and `process.kill(-pid)` for group termination.
- **[IMP][PERFORMANCE] Electron: orphan child processes on quit** — Closing the Electron app now kills the entire process group (lorg, lorg, lorg-tool) instead of only the lorg process. Uses `detached: true` spawn and `process.kill(-pid)` for group termination.
...
- **Cook bundled in Electron app** — `cook` binary is now built and embedded in the Electron app alongside lorg, lorg, and lorg-tool.
...
- **Electron Desktop App** — Standalone desktop application wrapping the lorg web UI.
...
## [2026-MAR] - v0.26.0 - Repository Rename: `lorg-db` -> `lorg`

### Changed

- **Repository renamed** from `github.com/campbellcharlie/lorg-db` to `github.com/campbellcharlie/lorg`
- **Go module path updated** — All imports changed from `github.com/campbellcharlie/lorg-db/...` to `github.com/campbellcharlie/lorg/...`
...
- The old GitHub URL (`github.com/campbellcharlie/lorg-db`) will redirect to the new one
- Update your git remote: `git remote set-url origin git@github.com:campbellcharlie/lorg.git`
- If using `go get`: `go get github.com/campbellcharlie/lorg@latest`
...
- **Self-Update Command** (1dc12df)
  - `lorg update` - Fetch and replace binaries (`lorg`, `lorg`, `lorg-tool`) from GitHub Releases
...
- **Electron App Launch Integration** (3a41c75)
  - Electron app now spawns `lorg start` as a child process on launch
...
- **Serve Configuration** (b3c5945)
  - Use `.lorg` directory and `chdir` on launch for cleaner working directory management
...
- **SDK for External Tools** (44a3971)
  ...
  - Environment variable support (`LORG_APP_URL`, `LORG_ADMIN_EMAIL`, `LORG_ADMIN_PASSWORD`)
...
- **Fuzzer Improvements** (44a3971, 553f762)
  ...
  - SDK integration for process tracking in external `lorg-tools`
...
- Frontend fetch improvements
- Changed `lorg-tool` current working directory handling
...
- Direct database integration using PocketBase `dao` instead of `lorgsdk`
...
- Fixed certificate location at `~/.config/lorg/`
